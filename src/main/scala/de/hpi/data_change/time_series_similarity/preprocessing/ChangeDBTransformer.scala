...
object ChangeDBTransformer extends App{
  ...
  def filterWeirdChanges(changeRecords:Array[ChangeRecord]) = {
    ...
    var skipped = 0
    for (cur <- changeRecords.sortBy(f => (f.entity, f.property, f.timestamp.toLocalDateTime.toEpochSecond(ZoneOffset.UTC)))) {
